<template>
    <div class="header">
        <a href="#/">
            <img class="logo" src="@/assets/login1.jpg" width="25px">
...
export default {
    ...
    data() {
        ...
        return {
            user: JSON.parse(localStorage.getItem('zhu-member-user')),
    ...
    methods: {
        ...
        // 退出系统
        handleLogout() {
            logout( localStorage.getItem('zhu-member-token') ).then(response => {
                const resp = response.data
                if(resp.flag) {
                    // 退出成功
                    // 清除本地数据
                    localStorage.removeItem('zhu-member-token')
                    localStorage.removeItem('zhu-member-user')
                    // 回到登录页面
